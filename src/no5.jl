...
!=#
# julia --code-coverage=user -e "include(\"no5.jl\"); using Base.Test; @test vecORdevec()"
#=!
...
!=#
# julia --track-allocation=user -e "include(\"no5.jl\"); using Base.Test; @test vecORdevec()"
#=!
...
=#
#=!
بررسی گزارش mem فایل تولید شده (کد بالا) نشان میدهد در حین محاسبه برداری 213023 واحد و غیر برداری 80048 واحد حافظه اختصاص داده میشود. در تحلیل گزارش فوق باید توجه داشت تعداد واحدهای اختصاص داده شده به خط اول تابع شامل حافظه اختصاص داده شده به کمپایلر و محیط تعاملی زبان جولیا (Julia REPL) نیز میباشد. 
!=#
